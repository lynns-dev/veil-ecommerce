...
// Backup checkout page, charging through QuickBooks Payments — not linked
// from anywhere on the live site (components/CartDrawer.jsx points at
// /checkout, which is pages/checkout.jsx, charging through Square instead).
// Kept as a ready-to-restore fallback so the site can rotate processors
// when necessary: to go live on QuickBooks again, swap which file lives at
// pages/checkout.jsx, the same way this page was moved out of it. Keep
// this page's non-payment sections in sync with checkout.jsx by hand when
// either one changes.
//
...
// Reads QB_ENVIRONMENT straight from the server on every request, rather
// than relying on a separately-configured NEXT_PUBLIC_QB_ENVIRONMENT that
// gets baked into the client bundle at build time. Two env vars for the
// same setting is a real footgun: if Vercel's Preview/Production scoping
// ever let them drift apart, the browser could tokenize a card against one
// Intuit environment (sandbox or production) while the server charges it
// against the other — producing exactly the confusing, deployment-specific
// errors ("account not enabled to take payments," inconsistent declines)
// seen while debugging this. Deriving it here guarantees the client always
// agrees with whatever lib/qbPaymentsServer.js is actually using.
