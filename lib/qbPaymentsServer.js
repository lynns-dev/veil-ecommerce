...
export async function chargeCard(token, amount) {
  ...
  const authResponse = await fetch(`${base()}/quickbooks/v4/payments/charges`, {
    ...
    // Reverted back to native booleans (was briefly changed to string
    // "false"/"true" based on a documented example that turned out not to
    // match this account's actual behavior) — confirmed live: a card that
    // charged successfully before that change started declining afterward
    // with the same card, same shopper, no other change in between. This
    // is the exact shape that was live and working.
    body: JSON.stringify({
  ...
  const captureResponse = await fetch(`${base()}/quickbooks/v4/payments/charges/${auth.id}/capture`, {
    ...
    // Reverted alongside the authorize call above, same reason.
    body: JSON.stringify({
      amount: Number(amount).toFixed(2),
      context: { mobile: false, isEcommerce: true },
    }),
