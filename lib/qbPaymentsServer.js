...
export async function chargeCard(token, amount) {
  ...
  const authResponse = await fetch(`${base()}/quickbooks/v4/payments/charges`, {
    ...
    // Native booleans (not the string "false"/"true" from one of Intuit's
    // documented examples) — tried the string form briefly, but declines
    // happened both before and after that change on the same card, so the
    // boolean/string shape was never actually the cause of any decline.
    // Kept as booleans since that's the natural JSON type for this field.
    body: JSON.stringify({
