...
// NEXT_PUBLIC_EMAIL_APP_URL is documented as a full origin
// (https://mail.veilpuff.com), but a bare domain pasted into Vercel
// without the scheme is an easy mistake to make and fails opaquely —
// fetch() rejects with "Failed to parse URL" rather than anything that
// points at the actual env var. Defaulting a missing scheme to https
// (this app is never deployed anywhere else) turns that misconfiguration
// into "it just works" instead of a support round-trip.
function emailAppBaseUrl() {
  const raw = process.env.NEXT_PUBLIC_EMAIL_APP_URL;
  if (!raw) return '';
