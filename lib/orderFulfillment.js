...
import { notifyOrderReceived } from './emailPlatform';
...
export async function fulfillOrder({ id, amount, items, eventId, url, req, paymentMethod, attribution, email, shipping, processor, captureId, shippingProtection }) {
  try {
    ...
    if (email) {
      ...
      // Tells the separate email platform (lib/emailPlatform.js) this
      // checkout converted, so it stops the abandoned_checkout automation
      // for this subscriber instead of emailing someone who already paid.
