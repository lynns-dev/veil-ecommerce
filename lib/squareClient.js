...
// against Square's own "Take a Card Payment" docs. Apple Pay and Google Pay
// below were NOT verified against primary docs this session — built from
// general knowledge of Square's Web Payments SDK. Each wallet is wrapped so
// a failure to initialize (unsupported browser/device, that wallet not
// enabled on the Square account, etc.) never breaks the rest of checkout —
// the calling page just doesn't show that button. The server-side
// CreatePayment call every token gets sent to (lib/squareServer.js) is
// also unverified — treat all of this as unverified until a real charge of
// each type has gone through end-to-end.
//
