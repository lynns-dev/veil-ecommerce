...
// checkout_payment/checkout_review are reached once a shopper advances to
// that step of the 3-step checkout (see lib/checkoutStage.js) — separate
// from checkout_start, which fires on landing on Step 1.
const EVENT_NAMES = ['pageview', 'addtocart', 'checkout_start', 'checkout_payment', 'checkout_review', 'purchase'];

// Adding several items, reloading /checkout, or jumping back and forth
// between steps shouldn't inflate these past one count per visitor — the
// admin funnel counts carts/checkout-steps per person, not per action.
const DEDUPED_EVENTS = ['addtocart', 'checkout_start', 'checkout_payment', 'checkout_review'];
