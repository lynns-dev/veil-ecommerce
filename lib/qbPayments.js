...
// server (keeps the site out of full PCI card-data scope) — no third-party
// iframe/SDK involved at all, just a fetch() call, unlike Square's
// SDK-rendered card element. The resulting one-time token is then sent to
// /api/qb-checkout, which charges it server-side via the QuickBooks
// Payments Charges API (lib/qbPaymentsServer.js).
