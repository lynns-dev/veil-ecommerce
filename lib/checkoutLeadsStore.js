...
// Upserts by email — typing the email field twice, entering checkout again
// through a different funnel page, or playing a popup game after already
// abandoning a checkout, all update the same entry instead of creating
// duplicates. A lead that's already 'purchased' never gets demoted back to
// 'abandoned' by a later call; phone, once given, is never cleared by a
// later call that didn't include one.
export async function recordLead({ email, phone, cart, source, sessionId, url, status = 'abandoned' }) {
  ...
  const updated = {
    ...existing,
    status: nextStatus,
    phone: trimmedPhone || existing.phone || null,
